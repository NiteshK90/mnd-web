...
export default function SectionCard({ title, quote, body, className = "" }: SectionCardProps) {
  return (
    <div className={`bg-white rounded-[28px] shadow-card flex flex-col gap-6 ${className}`}>
      <h3 className="font-canela text-[20px] font-semibold tracking-[-0.02em] text-mnd-charcoal">
        {title}
      </h3>
      ...
      {quote && (
        <div className="flex items-center gap-4 justify-start">
          ...
          <p className="w-[230px] font-sans text-sm italic font-normal leading-[1.3] text-mnd-charcoal">
            {quote}
          </p>
        </div>
      )}
      <div className="flex gap-4">
        ...
        <p className="w-[250px] font-sans text-[16px] font-bold leading-[1.35] text-mnd-charcoal">
          {body}
        </p>
      </div>
    </div>
